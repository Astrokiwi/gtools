...
#     linestyles = ['dashed','dashdot','solid']
    ...
    fig,sp = plt.subplots(ny,nx,figsize=(3*nx,3*ny),sharex=True,sharey=True,squeeze=False)
    phi_bin_edges = np.linspace(0.,90.,10)
...
#             print(ix,iy,sp.shape)
    ...
    for ix in range(nx):
        for iy in range(ny):
            sp[ix,iy].set_title(key_bases[iaxis][ix]+"_"+key_bases[jaxis][iy])
...
#             p.legend(loc='best',fontsize='xx-small')
    ...
    plt.savefig("../figures/vrad_phi_testflows.png",dpi=150)
    plt.close()
