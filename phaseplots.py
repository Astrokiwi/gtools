...
cs_p = np.sqrt(values["u_p"])
G = 6.67e-8 # in cgs
G_Jeansterm = G**1.5 * 1.989e33
values["mJ_p"] = 2./3.*(np.pi)**2.5 * cs_p**3/G_Jeansterm/np.sqrt(values["rho_p"])
...
#for iv in range(7):
#    for jv in [7]:
        fname = "../figures/"+run_id+output_dir+iv+jv+snap_str+".png"
        P.figure()
        P.xlabel(labels[iv])
        P.ylabel(labels[jv])
        if ( dolog[iv] ):
