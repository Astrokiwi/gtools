...
def calc_hist(run_id,output_dir,snap_str,dumpall,bins):
...
#         m_out = np.sum(outflowing)
        return int(snap_str),time,wind_theta,m_out
...
#         output_data = np.array([theta_edges[:-1],theta_histogram,grad,gradgrad]).T
    ...
    #         snapf = 999
            ...
            with Pool(processes=80) as pool:
                output_data = pool.starmap(calc_hist,zip(it.repeat(run_id),it.repeat(output_dir),snap_strs,it.repeat(False),it.repeat(bins)))
    #         print(output_data)
